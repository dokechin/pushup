    ...
    req.body.events.forEach((event) => {
        ...
        if (event.type == "message" && event.message.type == "text"){
            ...
            if (event.message.text == "今月"){
				...
				const query = {
				  text: 'SELECT menu.menu as menu,SUM(train.COUNT) as count FROM TRAIN inner join menu on train.menu_id = menu.menu_id WHERE train.execute_date between $1::timestamp and $2::timestamp train.user_id = $3 group by menu.menu',
				...
				const query = {
					text: 'SELECT menu.menu as menu,SUM(train.count) as count FROM train inner join menu on train.menu_id = menu.menu_id WHERE train.execute_date between $1::timestamp and $2::timestamp and train.user_id = $3 group by menu.menu',
					values: [firstDay,lastDay,event.source.userId],
				}
				pgclient.query(query, (err, res) => {
					if (err) {
